 ...
 * the storage folder will default to `~/.vecno`
...
/**
 *
 *  Vecno `Address` version (`PubKey`, `PubKey ECDSA`, `ScriptHash`)
 *
 * @category Address
 */
...
/**
 * Vecno Sighash types allowed by consensus
 * @category Consensus
 */
...
interface IGeneratorSettingsObject {
     ...
     * 
     * Typical usage: { address: "vecno:...", amount: 1000n }
     */
 ...
 * Wallet API for interfacing with Rusty Vecno Wallet implementation.
 */
...
export interface IHexViewConfig {
     ...
     * template is generated for mining in the Vecno BlockDAG.
     * 
     * @category Node RPC
     */
     ...
     * UTXO set override for the pruning point changes in the Vecno BlockDAG.
     * 
     * @category Node RPC
     */
     ...
     * Difficulty Adjustment Algorithm (DAA) score changes in the Vecno BlockDAG.
     * 
     * @category Node RPC
     */
     ...
     * score of the sink block changes in the Vecno BlockDAG.
     * 
     * @category Node RPC
     */
     ...
     * Vecno BlockDAG. The event notification is scoped to the
     ...
     * conflict in the Vecno BlockDAG is resolved.
     * 
     * @category Node RPC
     */
     ...
     * conflict occurs in the Vecno BlockDAG.
     * 
     * @category Node RPC
     */
     ...
     * chain changes in the Vecno BlockDAG.
     * 
     * @category Node RPC
     */
     ...
     * block is added to the Vecno BlockDAG.
     * 
     * @category Node RPC
     */
...
/**
 * Vecno [`Address`] struct that serializes to and from an address format string: `vecno:qz0s...t8cv`.
 *
 * @category Address
 */
...
*/
   ...
   * Receives a [`NetworkType`](vecno_consensus_core::network::NetworkType)
 ...
 * This class accepts the master Vecno XPrv string (e.g. `xprv1...`) and generates
 ...
 *
 *
 * Vecno RPC client uses ([wRPC](https://github.com/workflow-rs/workflow-rs/tree/master/rpc))
 * interface to connect directly with Vecno Node. wRPC supports
 * two types of encodings: `borsh` (binary, default) and `json`.
 *
 * There are two ways to connect: Directly to any Vecno Node or to a
 ...
 * ```
 *
 * **Connecting to a Vecno Node directly**
 *
 * ```javascript
 * let rpc = new RpcClient({
...
*/
  ...
  free(): void;
  /**
   * Retrieves the current number of blocks in the Vecno BlockDAG.
   ...
   * structure of the Vecno BlockDAG.
  ...
  /**
   * Returns the total current coin supply of Vecno network.
  ...
  /**
   * Provides a list of addresses of known peers in the Vecno
   ...
   * the highest cumulative difficulty in the Vecno BlockDAG.
  ...
  /**
   * Retrieves information about the Vecno server.
   * Returned information: Version of the Vecno server, protocol
   * version, network identifier.
  ...
  /**
   * Retrieves the balance of a specific address in the Vecno BlockDAG.
  ...
  /**
   * Retrieves balances for multiple addresses in the Vecno BlockDAG.
  ...
  /**
   * Retrieves a specific block from the Vecno BlockDAG.
  ...
  /**
   * Retrieves multiple blocks from the Vecno BlockDAG.
  ...
  /**
   * Retrieves block headers from the Vecno BlockDAG.
  ...
  /**
   * Retrieves information about a subnetwork in the Vecno BlockDAG.
  ...
  /**
   * Resolves a finality conflict in the Vecno BlockDAG.
   * Returned information: None.
  ...
  /**
   * Submits a block to the Vecno network.
   * Returned information: None.
  ...
  /**
   * Submits a transaction to the Vecno network.
  ...
  /**
   * Submits an RBF transaction to the Vecno network.
   ...
   * UTXOs changed notification event is produced when the set
   * of unspent transaction outputs (UTXOs) changes in the
   * Vecno BlockDAG. The event notification will be scoped to the
  ...
  /**
   * Connect to the Vecno RPC server. This function starts a background
  ...
  /**
   * Disconnect from the Vecno RPC server.
   */
...
*/
   ...
   * and any push of data greater than [`MAX_SCRIPT_ELEMENT_SIZE`](vecno_txscript::MAX_SCRIPT_ELEMENT_SIZE) will not modify
   ...
   * exceed the maximum allowed script engine size [`MAX_SCRIPTS_SIZE`](vecno_txscript::MAX_SCRIPTS_SIZE).
   */
 ...
 * on the Vecno network.  When an address is registered with UtxoContext
