...
use crate::stratum::jobs::{Jobs, Pending, PendingResult, JobParams};
use crate::treasury::sharehandler::Sharehandler;
use crate::vecnod::{VecnodHandle, RpcBlock};
use crate::database::db::Db;pub struct Stratum {
    pub last_template: Arc<tokio::sync::RwLock<Option<RpcBlock>>>,
    pub jobs: Arc<Jobs>,
    pub share_handler: Arc<Sharehandler>,
    pub pending: mpsc::UnboundedSender<PendingResult>,
    pub send: watch::Sender<Option<JobParams>>,
    pub payout_notify: broadcast::Sender<PayoutNotification>, // Changed to broadcast::Sender
}impl Stratum {
    pub async fn new(
        addr: &str,
        handle: VecnodHandle,
        ...
        let (send, _recv) = watch::channel(None);
        let (payout_notify, _) = broadcast::channel(100); // Initialize broadcast channel with capacity 100
        ...
        let worker_counter = Arc::new(AtomicU16::new(0));    let jobs_clone = jobs.clone();
    let share_handler_clone = share_handler.clone();
    let last_template_clone = last_template.clone();
    let pending_clone = pending.clone();
    let payout_notify_clone = payout_notify.clone(); // Clone sender for task
        ...
        loop {
            ...
            match listener.accept().await {
                Ok((mut stream, addr)) => {
                    ...
                    let payout_notify_recv = payout_notify_clone.subscribe(); // Subscribe to broadcast channel
                    ...
                    tokio::spawn(async move {
                        ...
                        let mut conn = StratumConn {
                            ...
                            payout_notify_recv, // Pass broadcast receiver
                        };
                        ...
                        if let Err(e) = conn.run().await {
                            ...
                            pending.resolve(Some(format!("Connection closed: {}", e).into()));
                        }
                    });
                }
